...
def main() -> int:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
        ...
        for client_index in range(args.max_clients):
            ...
            print(f"CLIENT_CONNECTED index={client_index} address={address}", flush=True)
            with connection:
                while True:
                    ...
                    try:
                        ...
                        state = np.asarray(request["state"], dtype=np.float32).reshape(1, 14)
                        raw = {
                            "pixels": {
                                key: images[key][None, ...]
                                for key in ("head_camera", "left_camera", "right_camera")
                        ...
                        action_chunk = postprocessor(action_chunk).detach().cpu().numpy()[0]
                        if action_chunk.shape != (50, 14) or not np.isfinite(action_chunk).all():
                            raise RuntimeError(
                        ...
                        send_message(
                            ...
                            {
                                "ok": True,
                                # Use Python containers across the Python 3.12 / NumPy 2
                                # policy process and Python 3.10 / NumPy 1.26 simulator
                                # process. Pickled ndarrays reference private NumPy
                                # modules whose names differ between those releases.
                                "actions": action_chunk.tolist(),
                                "latency_seconds": latency,
                                "state_dim": int(batch["observation.state"].shape[-1]),
                            },
                        )
