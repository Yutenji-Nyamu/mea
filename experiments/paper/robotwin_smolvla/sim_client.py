...
def resolved_demo_clean_args(task_name: str) -> dict:
    with open(
        os.path.join(CONFIGS_PATH, "demo_clean.yml"), encoding="utf-8"
    ) as stream:
...
def main() -> int:
    ...
    try:
        ...
        result = {
            ...
            "protocol": (
                "localhost length-prefixed pickle envelope with bytes/list payload; "
                "one 50-action chunk per request"
            ),
        }
