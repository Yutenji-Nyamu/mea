...
from mea.taskgen.artifacts import (
...
from mea.taskgen.click_bell import ClickBellTaskGenError, compile_click_bell_overlay
...
from mea.taskgen.scene_checks import SceneCheckSpecError, validate_scene_check_spec
from mea.taskgen.capabilities import CapabilityError, validate_variant_spec_envelope
from mea.taskgen.success_spec import SuccessSpecError, success_spec_validation_report
...
def run_bounded_task_generation(
    ...
    *,
    ...
    """Translate the current one-result TaskGen API for frozen protocols."""
    ...
    accepted = dict(result["accepted_result"])
    summary = {
        "schema_version": 1,
        "recovery_scope": "task_generation_before_policy",
        "proposal_identity": dict(proposal_identity),
        "status": "accepted",
        "max_regenerations": max_regenerations,
        "attempt_count": 1 + int(result.get("repair") is not None),
        "regenerations_used": int(result.get("repair") is not None),
        "attempts": [],
        "runtime": dict(result["runtime"]),
        "policy_retry_allowed": False,
        "accepted_result": accepted,
    }
    Path(attempt_root, "task_generation_attempt_summary.json").write_text(
