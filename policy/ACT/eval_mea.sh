...
if [ "$#" -lt 6 ] || [ "$#" -gt 16 ]; then
    echo "Usage: $0 TASK CONFIG CKPT EXPERT_NUM SEED GPU [NUM_EPISODES] [TASK_MODULE] [TASK_OVERLAY] [START_SEED] [TELEMETRY_DIR] [TELEMETRY_PROFILE] [SEED_MANIFEST] [SEED_RESULTS_PATH] [OUTPUT_DIR] [SHARED_ELIGIBILITY_MANIFEST]" >&2
    exit 2
fi
...
output_dir="${15:-}"
shared_eligibility_manifest="${16:-}"
