...
SEMANTIC CONCERN:
scene_robustness.lateral_shift.evidence_bracketed_terminal_tcp_margin: With the stapler initially translated by +0.030 m along world x, the policy will fail the satisfiable terminal conjunction of the official core predicate and left-TCP-to-stapler distance <= 0.07980 m, exposing an evidence-bracketed spatial-retargeting margin weakness.

REQUESTED SCENE NEED:
Retrieve or adapt the official press_stapler scene by translating only the stapler initial position from the official reset pose by +0.030 m along world x. Preserve unchanged: task identity; policy checkpoint; official core predicate as a required conjunct.

CHECKER NEED (context only; RGB cannot validate success logic):
Retrieve the official checker and create an experimental terminal checker with the boolean predicate: official core predicate as a required conjunct AND Euclidean distance(left_tcp_position, stapler_position) <= 0.07980 m.
