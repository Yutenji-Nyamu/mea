...
以下 Python/bash 验收命令必须在 canonical AutoDL repo 与 RoboTwin interpreter 中执行。Windows
staging 只做源码阅读、轻量编辑、changed-file 编译/静态检查和 `git diff --check`；本地单元测试、
RoboTwin import 或环境相关结果都不纳入正式验收。TaskGen、provider、expert、simulator 与 ACT 更不能
在 Windows staging 代替服务器结果。
...
## 34. Query contract、experimental TaskGen 与有效性协议

本节所有命令都在服务器 `/root/autodl-tmp/mea` 中运行。

### 34.1 0-ACT cached Query sufficiency

```bash
PYTHON=/root/autodl-tmp/conda/envs/RoboTwin/bin/python

"$PYTHON" scripts/manipeval_query_sufficiency.py \
  --repo-root "$PWD" \
  --task-name click_bell \
  --contract-json docs/evidence_runs/batch18_contracts/query_contract.json \
  --candidate-evidence-json docs/evidence_runs/batch18_contracts/query_candidate_evidence.json \
  --output-json mea/validation_runs/<new_query_assessment>.json
```

该 CLI 只读显式 JSON，绑定 task catalog 与 `BoundTaskPlanSession`，不调用 provider/simulator/ACT，也不
改变 live Planner。正式 evidence 中必须检查 `execution_mode=cached_offline_0_act` 和所有 call count=0。

### 34.2 public Proposal + experimental SuccessSpec v2
...
"$PYTHON" scripts/manipeval_proposal.py \
  --repo-root "$PWD" \
  --request 'Evaluate a bounded non-canonical BBH block color with an explicitly experimental success predicate.' \
  --task-name beat_block_hammer \
  --aspect-id object_appearance.color \
  --output-dir mea/validation_runs/<new_id> \
  --run-id run_<new_id> \
  --experimental-bbh-success-spec \
  --materialize
```

`--materialize` 只允许 TaskGen、render、rule、expert 与 production task acceptance；它固定
`act_rollouts_started=0`。experimental v2 当前 `act_runtime_eligible=false`，使用 `--run-act` 或从主
Agent 进入都会 fail closed。fresh/resume 的 ACT gate 以最终重建的 `TaskArtifactBundle` 为权威，不信任
manifest 中重复的 Proposal 副本。只有 official 与 experimental outcome 拥有独立字段后，才能讨论 ACT。

### 34.3 synthetic protocol smoke

```bash
"$PYTHON" scripts/manipeval_matched_efficiency.py \
  --synthetic-demo --output mea/validation_runs/<new_matched_smoke>.json

"$PYTHON" scripts/manipeval_independent_validity.py \
  --synthetic-demo --output mea/validation_runs/<new_validity_smoke>.json
```

两条命令只验证 preregistration、资源账本、结论比较、多人标注与 VQA control 算术，必须输出
`empirical_*_claim_eligible=false`。synthetic `2:1` 不能当成真实 sampling savings，synthetic
accuracy/AUROC 也不能当成人工有效性结果。
