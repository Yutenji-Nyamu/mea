...
### 5.1 Claim-first 自顶向下路线

后续路线默认持续从论文 claim 向下推导，不再机械地在“自底向上”和“自顶向下”之间轮换。
每个批次在开始前必须先写出它要使哪一条论文主张变得可检验，以及结束后会新增哪一个真实指标、
对照实验或完整方法案例。

所有候选工作分为三类：

1. **直接 claim 工作**：实现或实验直接对应 Abstract、Introduction contributions、Sec. 4 的三个
   research questions、Figs. 2–6 或 Tables 1–9；
2. **直接阻塞 claim 的工程**：若不完成就无法运行上述实验，且阻塞关系能具体指出；
3. **可选护栏与维护**：authority gate、hash、fail-closed、provenance、更多报告限制、测试扩充等。

默认顺序严格为 `1 > 2 > 3`。第三类可以作为批次验收条件，但不能单独占据论文复现路线，也不能
因为它让系统更安全或更可审计，就被计为论文方法或实验进展。若下一步主要在增加 gate、identity、
limitation 或测试数量，却没有让新的论文图、表或核心案例变得可运行，应立即停止并重新从论文
主张规划。

当前顶层问题按论文 Sec. 4 排序：

1. 少量动态采样能否在时间/样本显著降低时得到与完整 benchmark 一致的绝对结论和 policy 排名；
2. 无脚本提示的开放 Query 能否真实驱动多轮 sub-aspect、TaskGen、ToolGen 和后续细化；
3. RAG、visual self-check、README.Agent 等模块是否真的提高 TaskGen/ToolGen 生成成功率；
4. Plan Agent 与人工 sub-aspect、VQA 与人工视频判断、以及系统错误率是否达到论文报告的有效性。

代码整洁、恢复机制和可信边界只有在直接解锁这些问题时才提前。每批结束仍检查工程质量，但不再
用工程质量替代科学结果。
