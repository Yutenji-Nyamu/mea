...
TaskGen 场景错误已经走通 visual reject→diagnosis→repair→revalidate，BBH codegen 还能从受限
`SuccessSpec` 编译并验证 official-equivalent `check_success()`；跨任务 portfolio 父层能审核 completed
child，新 `EvaluationGraph` 还能为同一 Query 生成条件 child 计划，并根据 typed outcome 决定是否
激活下一 child。

这里的“具备”指 Fig. 2–5 主体数据流已有最小实现，不等于论文实验复现完成。portfolio 的
`reuse` 模式本身不会因果启动 child，EvaluationGraph CLI 当前也只产生和裁决 inert child command；
当前只评 ACT，任务族和受控轴很少，标注仍含
...
| Sec. 3.3.1、Fig. 3 | reuse-first TaskGen、任务/资产/文档 RAG、视觉自反思 | `TaskProposal`、shared capability catalog、`VariantSpec` v2、`SuccessSpec` v1、`mea/taskgen/`、scene validation | proposal 不含任意可执行字段且不能越过 capability；scene 与 success method 都声明来源；生成 success function 必须来自封闭 spec 并与可信语义差分一致 |
