...
`task/profile/aspect/checkpoint id`；通用证据合同可驱动 `drill_down / switch_aspect / stop`；
...
| --- | --- | --- | --- |
| Fig. 2、Sec. 3.2 | 开放问题驱动、多轮动态 Proposal | `mea/planner/`、`mea/history/`、`mea/portfolio.py` | 不能只从单任务固定模板中依次取值；前轮证据应真实改变后轮方向；跨任务父层应绑定 child 身份而不把 pipeline pass 当 policy success |
