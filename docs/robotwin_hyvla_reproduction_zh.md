...
任务均成功，也不是论文中的多策略排名或完整 MEA 方法证据。三次生产 MEA 接入尝试均在
policy 连接前因 binding 或 Plan Agent admission 停止，累计 0 rollout；它们不是
Hy-VLA policy failure。v2 的 provider typed needs 实际正确，根因是 schema-less
resolver 仍依赖旧 aspect 菜单；当前代码已移除该依赖。准确错误、命令和处置见冷流水账第 6 节。
...
生产接入尚未验收；不要把 standalone official N=1 写成已完成的 MEA round。
...
1. 将该 generic transport 收入 backend-neutral policy binding；
2. 先抽 10–15 个 official task 做 N=1 coverage，失败也原样记录；
3. 只在 control 可运行的任务上选择一个无手写 schema 的 MEA 干净旗舰；
