...
根 `README.md` 只保留项目定位、唯一生产入口、最小运行示例和本索引，不承载实验日志。
当前文档只保留以下入口：
...
四份 Hot 文档各有唯一职责：`architecture` 只写静态结构与数据流，`running guide`
只写命令和输出，`developer reference` 只写扩展接口，`paper claim/gap` 只写动态证据、
限制和下一步。其余文档不重复展开这些内容。
...
- **Prompt 迭代（Cold）**：真实失败、修正与定向回归只从
  [`experiments/paper/prompt_learning/README.md`](../experiments/paper/prompt_learning/README.md)
  进入；任务列表不进入 Hot 文档。旧 `mea/knowledge/{extractor.py,index.json}` 与
  `mea/retrieval/README.Agent.md` 仅服务兼容 retrieval/prototype，默认不作为当前生产
  Agent 的知识来源读取。
