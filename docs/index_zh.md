...
| 1 | BBH/click 共用 Adaptive Plan–Proposal 与 evidence replay 分支 | Sec. 3.2；Figs. 2/5 | source + synthetic replay | 0 ACT |
| 2 | 受限 SuccessSpec 生成并差分验证真正的 `check_success()` | Sec. 3.3.1；Fig. 3 | source + fixture + live provider codegen | 0 ACT |
| 3 | clean-head click_bell 两轮旗舰闭环与完整图文 evidence | Figs. 2–5 | live N=1×2 已完成 | 1–2 ACT |
| 4 | BBH 非预写变化；MetricSpec 扩展并完成注册/复用 | Figs. 3/4 | scale live codegen + cached real telemetry | 0–1 ACT |
| 5 | Query 驱动 EvaluationGraph，按 child evidence 决定下一任务 | Fig. 2；Secs. 3.2–3.4 | live plan + synthetic branch；自动 child orchestration 待补 | replay 0；live 约 2 ACT |
| 6 | adaptive/fixed matched N=1→N=3、微消融和小型 gold/扰动集 | Tables 1–3、6–8 | 仅既有 pilot/proxy；论文级证据待补 | 先 0，随后 2–4 ACT |

轮次 3 已证明当前实现能完成单任务主体闭环；轮次 5 仍差父图自动执行真实 child，补完后才可称
论文主体方法的小规模跨任务功能复现。轮次 6 才开始形成小规模论文实验复现证据。动态状态以
当前 evidence snapshot 和最新 development log 为准。
