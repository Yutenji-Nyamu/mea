...
| 1 | Query-conditioned sufficiency：分开 candidate universe、required coverage 与 budget cap | Sec. 3.2；Figs. 2/5 | clean-head v4 已完成 evidence-driven aspect switch，但以 hard cap=2 停止，right/base1 未测 | 先 0 ACT cached contract |
| 2 | 公共 Proposal 自然产生 scene + proposal-derived bounded SuccessSpec v2 | Sec. 3.3.1；Fig. 3 | reviewed Task exact reuse/acceptance 已有；公共 Proposal 仍主要 v1、official-preserving | 0 ACT fixtures，live 后置 |
| 3 | 当前动态 runtime 与 fixed 用同 Query/seed/最大预算比较结论与成本 | Figs. 2–5；Tables 1–3、9 | 旧 N=1 pilot savings=0；v4 是功能验收，不是 matched efficiency 结果 | 先 N=1，再 N=3 |
...
| 5 | 独立人工 gold、真实扰动 clips 与 VQA 鲁棒性 | Tables 6–8 | development-agent proxy/既有 pilot | 主链稳定后开始 |
