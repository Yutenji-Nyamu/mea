...
本项目的目标是在 RoboTwin 2.0 上，以 ACT 单任务策略和最小实验预算，逐步复现 ManipEvalAgent 的
...
尚未完成论文规模的多 policy、足量 seed、人工 gold 或完整统计表。
...
| 1 | 将 Query identity、完整 suite、P1 stop、P4 answer 与 P3 comparison 接入同一 live hash-bound chain | Sec. 3.2；Figs. 2/5 | P1 cached contract 与 P4 AnswerScope 已可执行；v4 replay 仍为 `inconclusive/continue`，尚未接入 live Planner | 先 0 ACT replay，再 1–2 ACT |
| 2 | official / experimental 双通道执行 proposal-derived bounded SuccessSpec v2 | Sec. 3.3.1；Fig. 3 | 公共 Proposal 已真实生成 scene + SuccessSpec，并通过 provider/simulator/expert 的 0-ACT acceptance；ACT 仍 fail closed | 先 cached telemetry，再最多 1 ACT |
| 3 | 用同一 structured P1 verdict 做真实 fixed/adaptive 结论与成本比较 | Figs. 2–5；Tables 1–3、9 | matched preregistration/资源账本已有；新协议只有 synthetic `2:1`/`2:2`，旧 live pair 节省为 0 | 先真实 N=1，再论文 5 trials/10 runs |
| 4 | 扩大 stage-specific TaskGen visual/success/code/expert recovery 覆盖 | Fig. 3；App. A.3.4 | 分阶段有界 recovery 已有；覆盖仍窄，论文不要求统一中央 controller | 0–1 expert |
| 5 | 独立人工 gold、真实扰动 clips 与 VQA 鲁棒性 | Tables 6–8 | 多 rater/control/accuracy/AUROC 协议已有；当前仍全 synthetic、无人类 gold | 主链稳定后开始 |
