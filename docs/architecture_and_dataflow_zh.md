...
原始 Query
  ...
  → TaskAdapter（任务 schema、checkpoint、official success 与可用 hooks 的唯一来源）
...
| 任务能力 | `mea/capability_adapter.py` | `TaskAdapter` 统一 task/schema/policy binding、official success、Planner/TaskGen/Tool/VQA hooks；避免多处重复任务枚举 |
...
`TaskAdapter` 是任务能力的唯一代码来源；official inventory/catalog 是由它导出的可检索
清单，不是另一套 Planner。发现某个 task 不等于当前 checkpoint 能执行它：单任务
checkpoint 只能在其声明任务上运行，除非另有可验证的 open-policy scope。若原始 Query
提出 catalog 外 concern，Planner 保留对应 `TaskNeed`/`ToolNeed`，但在没有可验证执行
能力前设置 `execution_authorized=false`，不得把“识别到需求”写成“已评价策略”。
论文消融、效率比较、人类/VQA
...
- `place_phone_stand` 的 expert N=1 成功而 ACT N=1 失败；该运行只证明第五个
  ...
  弱点或成功率结论。
- catalog-external mass Query 已在线以 0 ACT 识别为未注册 concern，并保留
  `TaskNeed`/`ToolNeed`；因为没有生成并验证可执行 scene/metric，
  `execution_authorized=false`。这证明开放需求边界，不证明 catalog 外 concern 已执行。
