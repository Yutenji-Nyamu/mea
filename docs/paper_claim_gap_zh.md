...
更多任务数量误当作论文核心贡献。实验表格的规模复现另算；本项目当前是 ACT 为主、一个未完成的
ACT/DP3 三 seed pilot、两个任务族和少量可信 capability 内的受限功能原型，不是论文规模或开放世界
能力的完整复现。
...
### 0.2 batch20 实证增量与当前结论

batch20 把若干“可执行接口”升级为真实 rollout 证据，但也产生了两个必须保留的负结果。完整边界见
[batch20 开发记录](development_log_20260723_batch20_integrated_claims_zh.md) 与
[hash 绑定的紧凑证据包](evidence_runs/batch20_integrated_claims/)。

| 论文主张 | batch20 的真实增量 | 当前判断 |
|---|---|---|
| Figs. 2/5：Query → evidence-conditioned planning → 回答 | 同一个 runtime 自动完成 official control、2 次 ACT、Rule/VQA/Aggregate、provider next step 与 Feedback；在预算尚余 1 轮时因 existential evidence sufficient 停止 | **有限域机制闭合**；Query 很容易被一个成功样本短路，尚未证明 ambiguity refinement 或开放发现 |
| Fig. 3：Proposal 生成 scene + `check_success()` 并裁决 rollout | provider 生成同一个 target+distractor `task.py`；checker fixture 3/3，通过 3 个 expert probe 并裁决 1 次 ACT | **单例机制闭合、语义证据未闭合**；结构仍受 exact-reference AST 强约束，seed 1012 的 checker=false 不是物理误击证据 |
| Fig. 4：新 Tool generate/validate/register/reuse | jerk Tool 完成 provider generation、3 个 oracle、run-local register 与 exact reuse | **生命周期机制闭合、policy evidence 缺失**；真实 ACT 无 target contact，数值为 null、validated episode 为 0 |
| Tables 1–2：更少时间/samples 且结论一致 | 独立 fixed 4 ACT 与 adaptive 2 ACT；少 2 次 rollout、170.30 秒和 470 policy steps | **负结果**；fixed 发现 position+instance 弱轴，adaptive 只发现 position，结论不一致，不能复现论文 claim |
| Table 9：少样本保持 policy ranking | ACT 完成 3/3，DP3 完成 2/3；共同两个 seed 都成功 | **未完成且不可排序**；总计 5/6 rollout，第三个 DP3 seed 在 expert gate 失败，pair order/Spearman 均为 null |
| Table 3：RAG/visual/README 提升生成成功率 | 一个 unseen proposal × 五条件都真实走 provider/codegen/static/render/expert/oracle | **只有 path smoke**；5/5 全过造成 ceiling，checker 仍非模型生成，不能估计消融效果 |

因此当前最优先的科学 gap 不是再加 receipt 或接口，而是：

1. **Query-conditioned stopping 是否保留 benchmark 结论。** 先用已有四候选证据做 0-ACT truth table，
   把 `existential`、`all`、`worst-axis`、`compare` 的充分性条件分开；随后用独立 seed 跑 6–7 ACT。
   正向判据必须同时满足结论一致、rollout/wall/policy-step 都减少。
2. **真正的 Fig. 5 ambiguity → refinement。** 只给非 existential 开放 Query；第二轮具体 sub-aspect
   必须在第一轮证据后才产生，预计 2–3 ACT。若仍按固定 catalog 顺序或因轮数上限停止，则不通过。
3. **完成可比较的 two-policy pilot。** 在任何 policy rollout 之前，用 expert-only gate 预筛一个
   候选 seed 池并冻结三个共同 eligible seeds，再跑 ACT/DP3 各 3 次；tie 或反向就停止并报告。
4. **让 Fig. 3/4 的反例和数值成为物理证据。** TaskGen 先补真实 distractor-contact 负例；
   ToolGen 先在已有 contact-positive telemetry 上 0-ACT 验证，不足时各最多 1 ACT。
5. **最后再扩统计面。** Table 3 补四个 proposals × 五条件；Plan/VQA 改为独立人工 gold；
   Fig. 6 用冻结、前瞻性 case 分母。它们都不能由 development-agent proxy 冒充论文结果。
...
| ---: | --- | --- | --- |
| 1 | Plan Agent 根据 `Query + Y1:t` 动态发现 sub-aspect（Sec. 3.2；Figs. 2/5） | batch20 在统一 runtime 内完成 official control→ACT→Rule/VQA→Aggregate→provider next step→第二次 ACT→Feedback，并因 existential evidence sufficient 提前停止 | 支持 axes 由 capability card 预先明示；只隐藏具体 candidate/template ID。Query 可被一个 success 短路，尚无 ambiguity→refinement 或开放 concern 发现 |
| 2 | TaskGen 对 Proposal 做 retrieve-or-generate，并交付 runnable scene + `check_success()`（Sec. 3.3；Fig. 3） | provider 生成同一个 target+distractor `task.py` 与 checker；fixture 3/3、三次 expert probe，并由 checker 裁决 1 次 ACT | 仍是 exact-reference AST 单例；seed1012 checker=false 但没有 physical distractor contact，尚无真实误击负例、多 proposal 成功率或 library reuse |
...
| 4 | ToolGen 根据 Tool Proposal 检索、生成、验证、注册与复用（Sec. 3.3；Fig. 4） | batch20 从自然 Query 生成受限 jerk Tool，三个 oracle 通过，run-local register 后相同 Query exact reuse 且 provider=false | lifecycle 单例成立；真实 ACT 无 target contact，value=null、validated_episode_count=0。没有 finite policy metric，也未改变下一轮 Planner |
...
| 6 | Aggregate 后的 evidence 改变后续规划并形成 Query-centric feedback（Secs. 3.2–3.4） | batch20 统一 runtime 中，official control evidence 触发 provider 选择 instance/base0，第二轮 evidence 触发充分性停止与 AnswerScope | 只证明 axis-primed finite existential contract；同 seed N=2，不证明非 existential Query 的覆盖、比较或最弱轴结论 |
...
| 8 | 少样本结论与 benchmark 一致，并评估 Planner/VQA（Tables 1–3、6–9） | batch20 独立 fixed 4/adaptive 2 得到真实 rollout、wall、policy-step 节省，但漏掉 instance 弱轴；ACT 完成 3/3、DP3 完成 2/3，共同 seeds 为 tie；另有一个 proposal×五条件 path smoke | Tables 1–2 得到负结果，Table 9 未完成；仍无 5 policy、N=3/5、独立人工 gold、可估计的真实 codegen ablation 或论文规模重复 |
...
| --- | --- | --- | --- |
| 核心结论 | 用显著更少的时间和 samples，仍得到与完整 simulation benchmark 可比的结论 | Abstract；Tables 1–2、4–5；每个 constructed task 默认 5 trials，ManipEvalAgent 完整流程重复 10 次 | **有真实节省但得到负结果**：独立 fixed 4/adaptive 2 少了 50% rollout、约 49.7% wall、约 49.9% policy steps，却漏掉 instance 弱轴。仍无 N=3/5、10 repetitions 或多任务结论 |
| 核心结论 | 少样本评估仍能保持五个 policy 的相对排名 | Table 9：ACT、DP、DP3、RDT、π0；RoboTwin/LIBERO clusters；10/20/50 rollouts；Spearman 与 bootstrap CI | **未完成且不可排序**：ACT 3/3、DP3 2/3 policy rollouts；共同两个 seed 都成功，第三个 DP3 seed expert-ineligible。pair order/Spearman=null；DP、RDT、π0、clusters 缺失 |
| 核心方法案例 | 开放 Query 在每轮依据 observations 动态选择或细化 sub-aspect，并在需要时现场生成 task/tool | Secs. 3.2–3.4；Figs. 2、5 | **有限域统一链已成立**：official control evidence 使 provider 选择 instance，第二轮后按 existential contract 停止并回答；但 controlled axes 预先明示，尚无 ambiguity refinement、开放 concern 发现或同链有效 Tool 数值 |
| 组件有效性 | TaskGen 生成 scene + `check_success()`，RAG、visual self-check、README.Agent 提升生成成功率 | Fig. 3；Table 3：complete 98%，各消融 93–96% | **provider-generated 单例存在，统计仍缺失**：target+distractor scene+checker 裁决 1 ACT；但 exact AST 限制强、物理误击负例缺失。一个 proposal×五条件全部通过只证明 path smoke，不能估计消融 |
| 组件有效性 | ToolGen 能现场生成、验证、注册并复用新 rule tool，RAG 提升成功率 | Fig. 4；Table 3：complete 96%，w/o RAG 92% | **lifecycle 单例存在，policy evidence 缺失**：新 jerk Tool 完成 generate/oracle/register/exact reuse；真实 ACT 为 null/no-target-contact、validated episode 为 0，也没有进入下一 Planner 决策 |
...
inactive-arm bug 已撤回，v3b 已用 bounded DSL active-arm 版本替代。当前缺口见 §0.2 与 §2–3。
...
## 5. 历史声明边界（batch18/19 与旧 v4）

本节保留各旧 artifact 当时的边界，不能覆盖 §0.2 的 batch20 当前汇总；其中“未接入统一 runtime”、
“actual saving=null”和“ACT/DP3 各 0/1”等描述只指对应的历史 artifact。
...
## 6. batch19 后的首要 gap（历史计划）

下表是 batch20 开始前的输入计划，若干项已由 batch20 部分执行；当前排序与最小下一步以 §0.2 为准。
