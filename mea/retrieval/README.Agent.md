...
2. Select an additional task only when its name strongly suggests a useful
   implementation pattern for the requested semantic change.
3. Do not keep task-specific examples permanently in this contract; select
   them from the current catalog only when the Query makes their pattern
   relevant.
